# Standardize TypeScript Module Imports for Prisma Client Core Libraries: Type Definition Files

These rules are ALWAYS ACTIVE for all TypeScript modules within the Prisma client packages, adapters, and integration test suites, specifically governing how type definition files (*.test-d.ts) and runtime components organize their module imports.

### Rules

- **R-PRISMA-001** SHOULD: Type definition files (*.test-d.ts) SHOULD import type utilities separately from runtime implementations to enable compile-time-only validation.
...
- Type definition files (*.test-d.ts) import type utilities separately from runtime implementations

<enforcement>
Claude Code MUST NOT skip or defer verification. All three verification commands MUST pass before accepting changes to TypeScript module imports in Prisma client packages, adapters, and integration test suites.
</enforcement>