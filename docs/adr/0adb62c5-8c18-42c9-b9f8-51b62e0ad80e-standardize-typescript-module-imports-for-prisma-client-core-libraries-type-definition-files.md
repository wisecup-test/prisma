...
1. SHOULD: Type definition files (*.test-d.ts) SHOULD import type utilities separately from runtime implementations to enable compile-time-only validation

## Policy Block

- SHOULD Type definition files (*.test-d.ts) SHOULD import type utilities separately from runtime implementations to enable compile-time-only validation
