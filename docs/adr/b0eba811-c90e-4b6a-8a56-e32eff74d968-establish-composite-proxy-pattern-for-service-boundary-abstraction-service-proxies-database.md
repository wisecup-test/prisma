...
1. SHOULD: Service proxies SHOULD be database-agnostic and support multiple backend implementations (PostgreSQL, SQLite, SQL Server) through the same interface

## Policy Block

- SHOULD Service proxies SHOULD be database-agnostic and support multiple backend implementations (PostgreSQL, SQLite, SQL Server) through the same interface
